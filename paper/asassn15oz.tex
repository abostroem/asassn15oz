...
It is useful to compare observations of ASASSN-15oz to those of other type IIP/IIL SNe to place our observations in the broader context of the SN field.
We start by looking for similar objects in our photometric and spectroscopic database, SNDAVIS \footnote{http://dark.physics.ucdavis.edu/sndavis/transient}. 
Although we identified SN 2016zb and 2015W based only on their $s_{50V}$ slope, they are surprisingly similar both photometrically and spectroscopically. 
Notably, the cachito feature is present in all three SNe and shows similar time evolution.
\Stefano{Should we even include this paragraph?}

%Slopes
To better understand the relationship between the photospheric slope and the radioactive decay slope and the physical mechanism for both, we compare the photospheric phase slope ($s_{50V}$) with the radioactive decay slope (Figure \ref{fig:SlopeComp}) for all SNe in our database with both slopes measured.
We compare this sample to all SNe in the database with either slope measured to ensure our sample is representative of the population.
...
This, indeed is what we see in Figure \ref{fig:SlopeComp}, that there is a correlation between photospheric and radioactive decay phase slopes.
\Stefano{I removed anything with an error greater than 0.25 (mag/50d), snhunt. is that too much data cleaning?}
%=======================
\begin{figure}
\begin{center}
...
%UV Excess
We also used the SNDAVIS database to search for a correlation between UV excess and light curve slope. 
If, in fact, the diversity in light curve shapes is due to mass loss and the interaction is strong enough to produce a UV excess, then those SNe with a steeper slope, should have lost more mass, and therefore should be more UV bright.
This should be most prominent during the first 30 days, when the CSM has the greatest affect on the light curve.
...
From the opical light curve we found the a compact CSM with a density of K = 10x10$^{17}$ g/cm extending to a radius of R = 2400 \rsun. 
...
Morozova \& Stone (submitted) modeled this scenario to reproduce both the optical light curves and the X-ray observations of SN 2013ej.
The X-ray luminosity observed for 2013ej is below our detection limits and therefore, with a similar model for ASASSN-15oz we would not expect an X-ray detection.
This scenario also supports our lack of narrow emission in our optical and NIR spectra as at the time of these observations the CSM in front of the shock was too low in density.
Finally, it is possible that the CSM interaction is not strong enough to produce a UV excess. \Stefano{is this true - we see U band excess in the SNEC LC, but we said that we don't trust those wavelengths b/c of line blanketing - can we say we don't expect the CSM interaction to be strong enough when we possibly see it in the model LCs?}
%-----------------------------------------
\section{Summary}
%%%%%%%%%%%%%%%%
