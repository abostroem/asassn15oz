...
Iair Arcavi,$\rm{^{9}}$
R\'egis Carier,$\rm{^{10}}$
Llu\'is Galbany,$\rm{^{11}}$
Claudia Guti\'errez,$\rm{^{12}}$
\newauthor
D. Andrew Howell,$\rm{^{13, 14}}$
Kristhell L\'opez,$\rm{^{15, 16}}$
Cosimo Inserra,$\rm{^{17}}$
Curtis McCully, $\rm{13, 14}$
Giuliano Pignata,$\rm{^{18, 19}}$
\newauthor
Anthony L. Piro,$\rm{^{20}}$
\'Osmar Rodr\'iguez,$\rm{^{21,22}}$
David R. Young,$\rm{^{23}}$
\\
% List of institutions
$\rm{^{1}}$University of California, Davis, 1 Shields Ave, Davis, CA, 95616 \\
$\rm{^{2}}$Hebrew University of Jerusalem \\ %Assaf
$\rm{^{3}}$Princeton University \\ %Viktoriya
$\rm{^{4}}$Mullard Space Science Laboratory, University College London, Holmbury St.~Mary, Dorking, Surrey RH5 6NT, UK\ \ %Paul
$\rm{^{5}}$University of Arizona \\ %Michael Lundquist, Dave, Sam
$\rm{^{6}}$Max-Planck Institut für Astrophysik, Karl-Schwarzschild Str. 1, D-85748 Garching, Germany \\ %Anders
$\rm{^{7}}$School of Physics and Astronomy, University of Southampton, Southampton, SO17 1BJ, UK\\ %Mark Sullivan, Mat Smith
$\rm{^{8}}$Harvard-Smithsonian Center for Astrohysics,  60 Garden Street,  Cambridge, MA 02138 \\ %Griffin
$\rm{^{9}}$ \\ % Iair
$\rm{^{10}}$Cerro Tololo Inter-American Observatory, National Optical Astronomy Observatory, Casilla 603, La Serena, Chile \\ %Regis Carier
$\rm{^{11}}$PITT PACC, Department of Physics and Astronomy, University of Pittsburgh, Pittsburgh, PA 15260, USA.\\%Lluís
$\rm{^{12}}$ \\ %Claudia
$\rm{^{13}}$Department of Physics, University of California, Santa Barbara, CA 93106-9530, USA\\ %Andy, Curtis
$\rm{^{14}}$Las Cumbres Observatory, 6740 Cortona Dr Ste 102, Goleta, CA 93117-5575, USA\\ %Andy, Curtis
$\rm{^{15}}$ SRON Netherlands Institute for Space Research, Sorbonnelaan 2 3584 CA Utrecht, The Netherlands\\ %Kristal
$\rm{^{16}}$ Department of Astrophysics/IMAPP, Radboud University, P.O. Box 9010, 6500 GL Nijmegen, The Netherlands\\ %Kristal
$\rm{^{17}}$\\ %Inserra
$\rm{^{18}}$Departamento de Ciencias Fisicas, Universidad Andres Bello, Avda. Republica 252, Santiago, Chile \\ %Pignata
$\rm{^{19}}$Millennium Institute of Astrophysics (MAS), Nuncio Monseñor Sótero Sanz 100, Providencia, Santiago, Chile \\ %Pignata
$\rm{^{20}}$ The Observatories of the Carnegie Institution for Science, 813 Santa Barbara Street, Pasadena, CA 91101, USA\\ %Tony
$\rm{^{21}}$ Departamento de Ciencias Fisicas, Universidad Andres Bello, Avda. Republica 252, Santiago, Chile\\ %Osmar
$\rm{^{22}}$ Millennium Institute of Astrophysics, Casilla 36-D, Santiago, Chile \\ %Osmar
$\rm{^{23}}$Astrophysics Research Centre, School of Mathematics and Physics, Queens University Belfast, Belfast BT7 1NN, UK\\ %David Young
}
...
We test this hypothesis on ASASSN-15oz, a type IIL supernova discovered by the All-Sky Automated Survey for SuperNovae (ASAS-SN) and the closest type II supernova in 2015. With extensive follow-up in the X-ray, UV, optical, IR, and radio we present our search for signs of interaction, and the mass-loss history indicated by their detection. 
...
We also detect the supernova in the radio, indicating typical red supergiant mass-loss rates of 10$\rm{^{-6}}$ \msunperiod/yr prior to the extreme mass-loss period.
Our failure to detect the supernova in the X-ray and the lack of narrow emission lines in the UV, optical, and NIR do not contradict this picture and place an upper limit on the mass-loss rate outside the extreme period of $\rm{10^{-4}}$ \msunperiod/yr.
...
However, the recent analysis of large samples of SNe by \citet{2014anderson} ({\it V}-band), \citet{2015sanders} ({\it grizy}), and \citet{2016galbany} ({\it UBVRIz}) show a continuum between the archetypical IIP and IIL light curves, strengthening the observations that earlier authors made on smaller samples (e.g. \citealt{1989young}, \citealt{1993patat}, and \citealt{1994patat}). 
...
From high resolution imaging prior to the SNe explosion, the progenitors of IIP/IIL SNe have been shown to be red supergiants (RSG) with masses $\rm{<}$ 17  \msun (\citealt{2015smartt} and references there in).
...
Observations that IIL-like SNe show shorter periods before falling to the radioactive decay phase, are brighter, and have higher velocity ejecta support this hypothesis \citep{1994patat,2014gutierrez,2014anderson,2014faran,2015sanders,2015valenti,2016valenti}.
...
Thus the mass loss rates for RSG are uncertain, with typical mass-loss rate estimates between $\rm{10^{-6}-10^{-4}}$ \msun/yr and velocities between $\rm{10-100}$ km/s \citep{2011mauron}.
...
\caption{The Swift UV spectrum from 2015-09-05 (JD 2457270.7; day 9), boxcar smoothed with a kernel of three pixels. 
...
We observed ASASSN-15oz using the VLA on 2015-09-24 (JD 2457289.6) and 2015-10-08 (JD 2457303.5). In both of the observations we used J1924-2914 and 3C48 as a phase calibrator and a flux calibrator, respectively. We used the Common Astronomy Software Applications (CASA) standard packages and pipelines to perform the data calibration and imaging. The SN was detected in both observation with the following flux measurements: September 24 - $\rm{120\pm 23\,\mu}$Jy at $\rm{4.8}$\,GHz (C-band) and a $\rm{3\,\sigma}$ non null detection limit at $\rm{22}$\,GHz (K-band); October 08 -  $\rm{210\pm 21\,\mu}$Jy at $\rm{4.8}$\,GHz (C-band) and a $\rm{80\pm 17\,\mu}$Jy at $\rm{15}$\,GHz (KU-band). 

%%%%%%%%%%%%%%%%% SN PARAMETERS %%%%%%%%%%%%%%%%%%
\section{Supernova Parameters} \label{15ozIntro}
SN ASASSN-15oz was discovered by the ASAS-SN team on 2015-08-31.09 (JD 2457265.59; V = 14.6 mag; RA = 19:19:33.55, Dec=-33:46:01.2) and announced on 2015-09-03 (JD 2457268; \citealt{2015brown}). 
The next day, 2015-09-04 (JD 2457270.0), LCO classified it as a Type II SN one week after explosion \citep{2015hosseinzadeh}. 
Figure \ref{fig:finder} shows the Digital Sky Survey (DSS) image of the low luminosity host galaxy, HIPASS J1919-33, prior to explosion (top panel) and our first {\it V}-band image of ASASSN-15oz taken on 2015-09-04 (JD 2457269.68).
Imaging of the host galaxy, HIPASS J1919-33, from 2015-08-23 (JD 2457257.56) does not show any evidence of ASASSN-15oz (V $\rm{>}$ 17.8; \citealt{2015brown}). 
We take this to be a lower limit on the explosion epoch and define the mid-point between the non-detection and the detection as explosion date with the $\rm{1\sigma}$ error being the time to the non-detection: $\rm{t_{expl} =}$ 2015-08-27 $\rm{\pm 4}$ days (JD 2457261.5). 
...
Bottom:{\it V}-band image of ASASSN-15oz taken by LCO eight days post explosion (2015-09-04).
...
The five best fit spectra imply an explosion epoch of  2015-08-22$\rm{\pm}$4.9 (JD 2457256.5), consistent with the range we propose based on non-detections.
...
As the uncertainties of both these methods are greater than that of the non-detection, we adopt the non-detection explosion epoch $\rm{t_{expl} =}$ 2015-08-27 $\rm{\pm 4}$ days (JD 2457261.5) throughout this paper, unless otherwise specified.

We use the Hubble flow distance modulus to the host galaxy, HIPASS J1919-33,  $\rm{\mu=32.3\pm 0.02}$, with H$\rm{_{0}}$=73 km/s/Mpc corrected for Virgo infall.
...
We adopt the \citet{2011schlafly}  value of 0.08 for the analysis in this paper and use the extinction law of \citet{1989cardelli}. 
%=======================
\begin{figure}
\begin{center}
...
The light curve of ASASSN-15oz reaches peak brightness in the {\it V}-band 8.25 days after explosion, one day after observations began, reaching an above average absolute magnitude of -18.05 $\rm{\pm}$ 0.025 mag.
...
The slopes we fit as well as the details of the fit are summarized in Table \ref{tab:slope} and plotted on top of the {\it V}-band light curve in Figure \ref{fig:slope}.
Both the high slopes and bright maximum luminosity of ASASSN15oz are similar to those of other IIL-like SNe. 
%=======================
\begin{table*}
\centering
\caption{The best-fit slope to the {\it V}-band light curve of ASASSN-15oz measured between the start and end phase listed in the table. All slopes are measured in units of magnitudes per 50 days.}\label{tab:slope}
\begin{tabular}{ccccc}
Slope Type & Slope (mag/50 days) & Slope Error & Start Phase (day) & End Phase (day) \\
\hline
$\rm{s_1}$ & 1.21 & 0.066 & 21.0 & 38.0 \\
$\rm{s_2}$ & 0.99 & 0.021 & 45.0 & 78.0 \\
$\rm{s_{50V}}$ & 1.09 & 0.014 & 18.4 & 78.0 \\
$\rm{s_{tail}}$ & 0.70 & 0.029 & 206.9 & 368.0 \\
\end{tabular}
\end{table*}%created by lightcurve_characteristics.ipynb
%-----------------------------------------
...
\caption{The $\rm{s_{1}, s_{2}, s_{50V}, and s_{tail}}$ fits plotted over the period used to derive the relation ($\rm{t_{start} - t_{stop}}$). 
...
To summarize the method, each apparent magnitude is corrected for galactic extinction (see Section \ref{15ozIntro}).
...
Identification was confirmed using the spectrum synthesis code Syn++ \footnote{\citealt{2013thomas}}\citep{2011thomas} (parameter details in Table \ref{tab:syn++}). 
%=======================
\begin{table*}
...
Ion & log($\rm{\tau}$) & $\rm{v_{min}(kkm/s)}$ & $\rm{v_{max}(kkm/s)}$ & aux & Temperature (kK) \\
\hline
...
\caption{A comparison of the velocity of ASASSN-15oz to the mean velocity of 122 IIP/IIL SNe \citep{2017gutierrez} for H-$\rm{\alpha}$ (left), H-$\rm{\beta}$ (middle), and Fe II ($\rm{\lambda 5169}$; right).
...
\caption{The nebular spectra of ASASSN-15oz from 2016-04-11 (JD 2457489.9; day 228; top) and 2016-08-03 (JD 2457603.7; day 340; bottom) plotted with the scaled models of \citet{2014jerkstrand} for $\rm{M_{ZAMS}}$ = 12, 15, 19, and 25 M$\rm{_{\odot}}$. 
...
To determine an upper limit, we sum all observations and find a limiting count rate of  $\rm{7.234\times10^{-4}}$ counts/s.
Assuming Galactic absorption of 6.18x$\rm{10^{20}}$ cm$\rm{^{-2}}$ \citep{2005kalberla}, this corresponds to an unabsorbed flux of $\rm{2.208\times10^{-14}}$ erg/cm$\rm{^{2}}$/s (0.4-5 keV) and a luminosity upper limit of $\rm{3.72\times10^{39}}$erg/s at 30 Mpc.

We compare this upper limit to the model of SN 1999em in \citet{2007chugai} which has similar progenitor and explosion parameters. 
...
\citet{2007chugai} find that the reverse shock dominates the X-ray observations for a pre-explosion mass loss rate of $\rm{10^{-6}}$ \msun/yr and a wind velocity of $\rm{10}$ km/s producing luminosities between $\rm{10^{38}}$ and $\rm{10^{39}}$ ergs/s. 
The forward shock produces luminosities comparable to the reverse shock in the first 10 days and decreasing to below $\rm{10^{34}}$ ergs/s by day 100.  
...
This spectrum shows no sign of the narrow emission lines typically associated with strong interaction (mass-loss rates of $\rm{\dot{M}>10^{-4}}$ g/cm; \citet{2012kiewe})
%%%%%%%%%%%%%%%%
\subsection{Radio}
...
We use Equation 1 from \citet{1998chevalier} to model the radio dataset and derive the following properties: the shockwave velocity (assuming constant velocity) is $\rm{\approx 1.4\times10^{4}}$ km/s. 
...
We find $\rm{K = 4.51\times 10^{11}}$ g/cm.
Assuming a wind velocity of 100 km/s, this implies a mass-loss rate of $\rm{\dot{M}}$ $\rm{\approx 0.9\times 10^{-6}}$ \msun/yr.
%%%%%%%%%%%%%%%%
...
We find no evidence for a high velocity feature in H-$\rm{\beta}$ nor HeI.
...
\caption{A comparison of the velocity of the Cachito feature if it is Si II ($\rm{\lambda6355}$) (navy triangles) and the velocity of the metals in the ejecta as characterized by the Fe II $\rm{\lambda5169}$ lines (cyan circles).
...
Opacities are drawn from OPAL type II opacity tables \citep{1996iglesias} at high temperatures ($\rm{T=10^{4.5}-10^{8.7}}$) and tables of \citet{2005ferguson} at low temperatures ($\rm{T = 10^{2.7}-10^{4.5}K}$) supplemented with an opacity floor of 0.01 cm$\rm{^2}$g$\rm{^{-1}}$.
...
However, for the typical mass loss rates of $\rm{\lesssim10^{-6}}$ \msun/yr,  the density in these regular steady winds is too low to have any noticeable effect on the post-explosion optical light curves, and these winds are not included in the progenitor profiles. 
...
CSM Density (10$\rm{^{17}}$ g/cm) & {\bf 10}, 20, 30, 35, 40, 50, 60 \\
...
We find the light curve of ASASSN-15oz is best characterized by $\rm{M_{ZAMS} = 18}$ \msunperiod, $\rm{E_{exp}= 1.4\times10^{51}}$ ergs, $\rm{K = 1.0\times10^{18}}$ g/cm, $\rm{R_{ext} = 2400}$ \rsun, $\rm{M_{Ni} = 0.083}$  \msunperiod, $\rm{t_{offset} = -4}$ days.
...
\caption{The model photospheric velocity computed by SNEC (navy) compared to the observed photospheric velocity (teal) taken to be the velocity of the Fe II $\rm{\lambda}$5169 feature. 
The model deviated from the observations around the time that the photosphere begins to recede into the ejecta.
The model velocity at $\rm{\tau_{sob}=1}$ is plotted in cyan. 
...
Using our database of publicly available light curves, SNDAVIS \footnote{http://dark.physics.ucdavis.edu/sndavis/transient}, we plot the photospheric slope ($s\rm{_{50V}}$) and the absolute {\it V}-band magnitude for 105 SNe (see Figure \ref{fig:SlopeMag}). 
...
\caption{The absolute {\it V}-band magnitude of a sample of public SNe from the SNDAVIS database compared to their photospheric slope ($\rm{s_{50V}}$) (navy circles). 
...
%Slopes
We also compare the photospheric phase slope ($\rm{s_{50V}}$) with the radioactive decay slope (Figure \ref{fig:SlopeComp}) for ASASSN-15oz with all SNe in our database for which both slopes are measured.
...
ASASSN-15oz is plotted in rose.
...
The dotted grey line represents the traditional separation between IIP and IIL SNe with a slope of $\rm{s_{50V}=0.5}$ mag (50 day)$\rm{^{-1}}$}
\label{fig:SlopeComp}
\end{center}
\end{figure}
%-----------------------------------------
...
From the optical light curve we found the a compact CSM with a density of $\rm{K = 1.0\times10^{18}}$ g/cm extending to a radius of R = 2400 \rsun. 
Assuming a typical of RSG wind velocity of 100 km/s, the CSM density required to fit the optical component implies a mass-loss rate of 0.15 \msun/yr.
This is significantly higher than radio mass-loss rate, $\rm{10^{-6}}$ \msun/yr.
While the mass-loss rate from the radio observations is consistent with typical RSG mass-loss rates of $\rm{10^{-4} }$- $\rm{10^{-6}}$ \msun/yr, the optical mass-loss rate is not inconsistent with some late stage mass-loss scenarios (e.g. \citealt{2012quatert,2014shiode,2017fuller, 2017yaron}).
...
\begin{itemize}
\item We detect the SN in radio observations, implying a mass-loss rate of $\rm{0.9\times10^-6}$ \msunperiod/yr for a wind velocity of 100 km/s.
...
\item To accurately model the early optical light curve, we require CSM around the RSG progenitor with a density of $\rm{10^{18}}$ g/cm and radial extent of 2400 $\rm{R_{\odot}}$, implying a short period of extreme mass-loss (e.g. assuming a wind velocity of 100 km/s, the mass-loss rate would be 2 \msunperiod/yr for 0.25 yrs).
\item No narrow emission lines are detected in the optical and UV photospheric spectroscopy, implying a mass-loss rate of $\rm{< 10^{-4}}$ \msunperiod/yr.
\item No UV excess is detected in the first 40 days of observations.
\item No X-ray signal is detected in the sum of all photospheric observations, implying $\rm{L_{xray}<3.72\times10^{39}}$ erg/s.
\end{itemize}
...
Based on observations collected at the European Organization for Astronomical Research in the Southern Hemisphere under ESO program 095.A-0316(A). We acknowledge support from EU/FP7-ERC grant [615929].\\
...
There's an LCOGT in the acknowledgments that should be converted to LCO.  Please also include: DAH, GH and CM are supported by NSF grant AST-1313484.\\
